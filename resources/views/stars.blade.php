...
</div>@if(count($stars) >=1) <!-- condition pour ne pas afficher d'erreur si pas de stars en bdd -->
<div class="flex" x-data="tabs('tab{{$stars[0]->id}}')"> <!-- Permet d'afficher le premier element récuperer de la bdd -->
    <div class="flex flex-col">
        ...
        @endforeach <!-- la fonction toggleTab permet d'afficher l'element sur le quel on click en changeant la valeur de tab -->
    </div>
    <div class="h-full"> <!-- Je reboucle sur mon tableau de données pour séparer les div pour le "design" -->
        @foreach($stars as $star)
        <template x-if="isActive('tab{{$star->id}}')"> <!-- isActive permet d'afficher l'element actif -->
